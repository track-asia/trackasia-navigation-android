...
public class NavigationTrackAsiaMap {
   ...
   * @param trackAsiaMap for APIs to interact with the map
   */
  public NavigationTrackAsiaMap(@NonNull MapView mapView, @NonNull TrackAsiaMap trackAsiaMap) {
    this.mapView = mapView;
    this.trackAsiaMap = trackAsiaMap;
    initializeLocationComponent(mapView, trackAsiaMap);
    initializeMapPaddingAdjustor(mapView, trackAsiaMap);
    initializeNavigationSymbolManager(mapView, trackAsiaMap);
    initializeMapLayerInteractor(trackAsiaMap);
    initializeRoute(mapView, trackAsiaMap);
    initializeCamera(trackAsiaMap, locationComponent);
    initializeLocationFpsDelegate(trackAsiaMap, locationComponent);
  }
  ...
  // Package private (no modifier) for testing purposes
  NavigationTrackAsiaMap(TrackAsiaMap trackAsiaMap, MapLayerInteractor layerInteractor, MapPaddingAdjustor adjustor) {
    this.layerInteractor = layerInteractor;
    initializeWayName(trackAsiaMap, adjustor);
  }
   ...
   * <p>
   * These will automatically be removed in {@link TrackAsiaNavigation#onDestroy()}.
   *
   * @param navigation to add the progress listeners
   */
  public void addProgressChangeListener(@NonNull TrackAsiaNavigation navigation) {
    initializeWayName(trackAsiaMap, mapPaddingAdjustor);
  ...
  private void initializeMapPaddingAdjustor(MapView mapView, TrackAsiaMap trackAsiaMap) {
    mapPaddingAdjustor = new MapPaddingAdjustor(mapView, trackAsiaMap);
  }

  private void initializeNavigationSymbolManager(MapView mapView, TrackAsiaMap trackAsiaMap) {
    Bitmap markerBitmap = ThemeSwitcher.retrieveThemeMapMarker(mapView.getContext());
    trackAsiaMap.getStyle().addImage(NavigationSymbolManager.TRACKASIA_NAVIGATION_MARKER_NAME, markerBitmap);
    SymbolManager symbolManager = new SymbolManager(mapView, trackAsiaMap, trackAsiaMap.getStyle());
    navigationSymbolManager = new NavigationSymbolManager(symbolManager);
    SymbolOnStyleLoadedListener onStyleLoadedListener = new SymbolOnStyleLoadedListener(trackAsiaMap, markerBitmap);
    mapView.addOnDidFinishLoadingStyleListener(onStyleLoadedListener);
  }

  private void initializeMapLayerInteractor(TrackAsiaMap trackAsiaMap) {
    layerInteractor = new MapLayerInteractor(trackAsiaMap);
  }
  ...
  private void initializeWayName(TrackAsiaMap trackAsiaMap, MapPaddingAdjustor paddingAdjustor) {
    if (mapWayName != null) {
      return;
    }
    initializeStreetsSource(trackAsiaMap);
  ...
  private void initializeStreetsSource(TrackAsiaMap trackAsiaMap) {
