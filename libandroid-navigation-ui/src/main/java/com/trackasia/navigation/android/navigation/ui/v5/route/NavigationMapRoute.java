...
public class NavigationMapRoute implements LifecycleObserver {
  ...
  private TrackAsiaNavigation navigation;
   ...
   * @param trackAsiaMap the TrackAsiaMap to apply route with
   * @since 0.4.0
   */
  public NavigationMapRoute(@NonNull MapView mapView, @NonNull TrackAsiaMap trackAsiaMap) {
    this(null, mapView, trackAsiaMap, R.style.NavigationMapRoute);
  }
  ...
  public NavigationMapRoute(@NonNull MapView mapView, @NonNull TrackAsiaMap trackAsiaMap,
    ...
    this(null, mapView, trackAsiaMap, R.style.NavigationMapRoute, belowLayer);
  }

  /**
   * Construct an instance of {@link NavigationMapRoute}.
   *
   * @param navigation an instance of the {@link TrackAsiaNavigation} object. Passing in null means
   *                   your route won't consider rerouting during a navigation session.
   * @param mapView    the MapView to apply the route to
   * @param trackAsiaMap  the TrackAsiaMap to apply route with
   * @since 0.4.0
   */
  public NavigationMapRoute(@Nullable TrackAsiaNavigation navigation, @NonNull MapView mapView,
                            @NonNull TrackAsiaMap trackAsiaMap) {
    this(navigation, mapView, trackAsiaMap, R.style.NavigationMapRoute);
  }
   ...
   */
  public NavigationMapRoute(@Nullable TrackAsiaNavigation navigation, @NonNull MapView mapView,
                            @NonNull TrackAsiaMap trackAsiaMap, @Nullable String belowLayer) {
    this(navigation, mapView, trackAsiaMap, R.style.NavigationMapRoute, belowLayer);
  }
   ...
   */
  public NavigationMapRoute(@Nullable TrackAsiaNavigation navigation, @NonNull MapView mapView,
                            @NonNull TrackAsiaMap trackAsiaMap, @StyleRes int styleRes) {
    this(navigation, mapView, trackAsiaMap, styleRes, null);
  }
  ...
  public NavigationMapRoute(@Nullable TrackAsiaNavigation navigation, @NonNull MapView mapView,
    ...
    this.routeLine = buildMapRouteLine(mapView, trackAsiaMap, styleRes, belowLayer);
    this.routeArrow = new MapRouteArrow(mapView, trackAsiaMap, styleRes);
  ...
  // For testing only
  NavigationMapRoute(@Nullable TrackAsiaNavigation navigation, @NonNull MapView mapView,
                     @NonNull TrackAsiaMap trackAsiaMap, @StyleRes int styleRes, @Nullable String belowLayer,
   ...
   * the progress updates from {@link TrackAsiaNavigation}.
   * <p>
   * If a new route is given to {@link TrackAsiaNavigation#startNavigation(DirectionsRoute)}, this
   * class will automatically draw the new route.
   *
   * @param navigation to add the progress change listener
   */
  public void addProgressChangeListener(TrackAsiaNavigation navigation) {
    this.navigation = navigation;
  ...
  /**
   * Should be called if {@link NavigationMapRoute#addProgressChangeListener(TrackAsiaNavigation)} was
   * called to prevent leaking.
   *
   * @param navigation to remove the progress change listener
   */
  public void removeProgressChangeListener(TrackAsiaNavigation navigation) {
    if (navigation != null) {
  ...
  private MapRouteLine buildMapRouteLine(@NonNull MapView mapView, @NonNull TrackAsiaMap trackAsiaMap,
    ...
    return new MapRouteLine(context, trackAsiaMap.getStyle(), styleRes, belowLayer,
  ...
  private void addListeners() {
    if (!isMapClickListenerAdded) {
      trackAsiaMap.addOnMapClickListener(mapRouteClickListener);
  ...
  private void removeListeners() {
    if (isMapClickListenerAdded) {
      trackAsiaMap.removeOnMapClickListener(mapRouteClickListener);
  ...
  private void redraw(Style style) {
    routeArrow = new MapRouteArrow(mapView, trackAsiaMap, styleRes);
