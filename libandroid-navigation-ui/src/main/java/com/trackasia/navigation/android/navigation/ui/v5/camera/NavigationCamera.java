 ...
 * <p>
 * This class listens to the progress of {@link TrackAsiaNavigation} and moves
...
public class NavigationCamera implements LifecycleObserver {
   ...
   *
   * @param trackAsiaMap         for moving the camera
   * @param navigation        for listening to location updates
   * @param locationComponent for managing camera mode
   */
  public NavigationCamera(@NonNull TrackAsiaMap trackAsiaMap, @NonNull TrackAsiaNavigation navigation,
                          @NonNull LocationComponent locationComponent) {
    this.trackAsiaMap = trackAsiaMap;
    this.navigation = navigation;
    this.locationComponent = locationComponent;
    this.animationDelegate = new CameraAnimationDelegate(trackAsiaMap);
   ...
   * @param locationComponent for managing camera mode
   */
  public NavigationCamera(@NonNull TrackAsiaMap trackAsiaMap, LocationComponent locationComponent) {
    this.trackAsiaMap = trackAsiaMap;
    this.locationComponent = locationComponent;
    this.animationDelegate = new CameraAnimationDelegate(trackAsiaMap);
    this.locationComponent.addOnCameraTrackingChangedListener(cameraTrackingChangedListener);
   ...
   */
  NavigationCamera(TrackAsiaMap trackAsiaMap, TrackAsiaNavigation navigation, ProgressChangeListener progressChangeListener,
  ...
  /**
   * This method can be called if you did not pass an instance of {@link TrackAsiaNavigation}
   * to the constructor.
   * <p>
  ...
  public void addProgressChangeListener(TrackAsiaNavigation navigation) {
    this.navigation = navigation;
    navigation.setCameraEngine(new DynamicCamera(trackAsiaMap));
    navigation.addProgressChangeListener(progressChangeListener);
  }
  ...
  private void initializeWith(TrackAsiaNavigation navigation) {
    navigation.setCameraEngine(new DynamicCamera(trackAsiaMap));
    updateCameraTrackingMode(trackingCameraMode);
  }
  ...
  private void animateTrackAsiaMapForRouteOverview(int[] padding, List<Point> routePoints) {
    ...
    trackAsiaMap.animateCamera(resetUpdate, 150,
      new CameraOverviewCancelableCallback(overviewUpdate, trackAsiaMap)
    );
  }
  ...
  private long getZoomAnimationDuration(double zoom) {
    double zoomDiff = Math.abs(trackAsiaMap.getCameraPosition().zoom - zoom);
  ...
  private long getTiltAnimationDuration(double tilt) {
    double tiltDiff = Math.abs(trackAsiaMap.getCameraPosition().tilt - tilt);
    return (long) MathUtils.clamp(
      500 * tiltDiff,
