 ...
 */
public class TrackAsiaRouteFetcher extends RouteFetcher {
    ...
    public TrackAsiaRouteFetcher(Context context) {
    ...
    private void addDestination(List<Point> remainingWaypoints, NavigationRoute.Builder builder) {
        if (!remainingWaypoints.isEmpty()) {
            builder.destination(toTrackAsiaPoint(retrieveDestinationWaypoint(remainingWaypoints)));
        }
    }
    ...
    private void addWaypoints(List<Point> remainingCoordinates, NavigationRoute.Builder builder) {
        if (!remainingCoordinates.isEmpty()) {
            for (Point coordinate : remainingCoordinates) {
                builder.addWaypoint(toTrackAsiaPoint(coordinate));
            }
        }
    }
     ...
     * any {@link RouteListener} that has been added via {@link TrackAsiaRouteFetcher#addRouteListener(RouteListener)}.
     *
    ...
    private List<Point> toMapboxPointList(List<com.trackasia.geojson.Point> pointList) {
        List<Point> mapboxPointList = new ArrayList<>();
        for (com.trackasia.geojson.Point point : pointList) {
            mapboxPointList.add(toMapboxPoint(point));
        }
        return mapboxPointList;
    }

    private Point toMapboxPoint(com.trackasia.geojson.Point point) {
        return Point.fromLngLat(point.longitude(), point.latitude());
    }

    private List<com.trackasia.geojson.Point> toTrackAsiaPointList(List<Point> pointList) {
        List<com.trackasia.geojson.Point> mapboxPointList = new ArrayList<>();
        for (Point point : pointList) {
            mapboxPointList.add(toTrackAsiaPoint(point));
        }
        return mapboxPointList;
    }

    private com.trackasia.geojson.Point toTrackAsiaPoint(Point point) {
        return com.trackasia.geojson.Point.fromLngLat(point.longitude(), point.latitude());
    }
}
