...
class MapRouteArrow {
  ...
  MapRouteArrow(MapView mapView, TrackAsiaMap trackAsiaMap, @StyleRes int styleRes) {
    ...
    arrowColor = typedArray.getColor(R.styleable.NavigationMapRoute_upcomingManeuverArrowColor,
      ContextCompat.getColor(context, R.color.trackasia_navigation_route_upcoming_maneuver_arrow_color));
    arrowBorderColor = typedArray.getColor(R.styleable.NavigationMapRoute_upcomingManeuverArrowBorderColor,
      ContextCompat.getColor(context, R.color.trackasia_navigation_route_upcoming_maneuver_arrow_border_color));
    typedArray.recycle();
  ...
  private void initialize() {
    ...
    trackAsiaMap.getStyle().addLayerBelow(shaftCasingLayer, RouteConstants.LAYER_ABOVE_UPCOMING_MANEUVER_ARROW);
    trackAsiaMap.getStyle().addLayerAbove(headCasingLayer, shaftCasingLayer.getId());

    trackAsiaMap.getStyle().addLayerAbove(shaftLayer, headCasingLayer.getId());
    trackAsiaMap.getStyle().addLayerAbove(headLayer, shaftLayer.getId());
  ...
  private void initializeArrowShaft() {
    ...
    trackAsiaMap.getStyle().addSource(arrowShaftGeoJsonSource);
  }
  ...
  private void initializeArrowHead() {
    ...
    trackAsiaMap.getStyle().addSource(arrowHeadGeoJsonSource);
  }
  ...
  private void addArrowHeadIcon() {
    ...
    trackAsiaMap.getStyle().addImage(RouteConstants.ARROW_HEAD_ICON, icon);
  }
  ...
  private void addArrowHeadIconCasing() {
    ...
    trackAsiaMap.getStyle().addImage(RouteConstants.ARROW_HEAD_ICON_CASING, icon);
  }

  private LineLayer createArrowShaftLayer() {
    LineLayer shaftLayer = (LineLayer) trackAsiaMap.getStyle().getLayer(RouteConstants.ARROW_SHAFT_LINE_LAYER_ID);
    if (shaftLayer != null) {
      trackAsiaMap.getStyle().removeLayer(shaftLayer);
    }
  ...
  private LineLayer createArrowShaftCasingLayer() {
    LineLayer shaftCasingLayer = (LineLayer) trackAsiaMap.getStyle().getLayer(RouteConstants.ARROW_SHAFT_CASING_LINE_LAYER_ID);
    if (shaftCasingLayer != null) {
      trackAsiaMap.getStyle().removeLayer(shaftCasingLayer);
    }
  ...
  private SymbolLayer createArrowHeadLayer() {
    SymbolLayer headLayer = (SymbolLayer) trackAsiaMap.getStyle().getLayer(RouteConstants.ARROW_HEAD_LAYER_ID);
    if (headLayer != null) {
      trackAsiaMap.getStyle().removeLayer(headLayer);
    }
  ...
  private SymbolLayer createArrowHeadCasingLayer() {
    SymbolLayer headCasingLayer = (SymbolLayer) trackAsiaMap.getStyle().getLayer(RouteConstants.ARROW_HEAD_CASING_LAYER_ID);
    if (headCasingLayer != null) {
      trackAsiaMap.getStyle().removeLayer(headCasingLayer);
    }
