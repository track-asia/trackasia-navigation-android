...
/**
 * Internal usage only, use navigation by initializing a new instance of [TrackAsiaNavigation]
 * and customizing the navigation experience through that class.
 *
 *
 * This class is first created and started when [TrackAsiaNavigation.startNavigation]
...
open class NavigationService : Service() {
    ...
    /**
     * This gets called when [TrackAsiaNavigation.startNavigation] is called and
     * setups variables among other things on the Navigation Service side.
     */
    fun startNavigation(trackAsiaNavigation: TrackAsiaNavigation, routeUtils: RouteUtils) {
        initialize(trackAsiaNavigation, routeUtils)

        trackAsiaNavigation.route
            ?.let { route ->
    ...
    /**
     * Called with [TrackAsiaNavigation.locationEngine].
    ...
    private fun initialize(trackAsiaNavigation: TrackAsiaNavigation, routeUtils: RouteUtils) {
        val notificationProvider = NavigationNotificationProvider(application, trackAsiaNavigation)
        this.notificationProvider = notificationProvider

        val thread =
            initializeRouteProcessorThread(trackAsiaNavigation.eventDispatcher, notificationProvider, routeUtils)
        initializeLocationProvider(trackAsiaNavigation, thread, routeUtils)
    }
    ...
    private fun initializeLocationProvider(
        ...
        val locationEngine = trackAsiaNavigation.locationEngine
        val listener = NavigationLocationEngineListener(
            trackAsiaNavigation = trackAsiaNavigation,
            validator = LocationValidator(trackAsiaNavigation.options.locationAcceptableAccuracyInMetersThreshold),
            thread = thread,
        )
