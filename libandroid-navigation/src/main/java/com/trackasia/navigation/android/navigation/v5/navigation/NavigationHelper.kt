...
object NavigationHelper {
    ...
    fun buildSnappedLocation(
        ...
        val location = if (!userOffRoute && snapToRouteEnabled) {
            trackAsiaNavigation.snapEngine.getSnappedLocation(rawLocation, routeProgress)
        } else {
            rawLocation
        }
        return location
    ...
    /**
     * This method runs through the list of milestones in [TrackAsiaNavigation.milestones]
     ...
     * @param trackAsiaNavigation      for list of milestones
    ...
    fun checkMilestones(
        ...
        return trackAsiaNavigation.milestones
     ...
     *
     *
     * If enabled, the off route engine is retrieved from [TrackAsiaNavigation] and
    ...
    fun isUserOffRoute(
        ...
        val options = navigationLocationUpdate.trackAsiaNavigation.options
        if (!options.enableOffRouteDetection) {
            return false
        }

        val offRouteEngine = navigationLocationUpdate.trackAsiaNavigation.offRouteEngine
    ...
    fun shouldCheckFasterRoute(navigationLocationUpdate: NavigationLocationUpdate, routeProgress: RouteProgress): Boolean {
        val fasterRouteEngine = navigationLocationUpdate.trackAsiaNavigation.fasterRouteEngine
